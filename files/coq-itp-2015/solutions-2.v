...
*)
Definition attempt_andb (b1 : bool) (b2 : bool) :=
  match b1, b2 with
  | true, true => true
  | _, _ => false
  end.

Eval compute in attempt_andb true false.
Eval compute in attempt_andb true true.


(* A better definition that minimized the number
   of pattern matching
 *)
...
(* To see the difference, let's compute on a
   symbolic expression: here the second argument
   passed to the boolean conjunction is a variable *)
Eval compute in (fun b => andb true b).
Eval compute in (fun b => attempt_andb true b).
...
(* We now define a function checking if an
   ...
   the type of "o"
*)
Definition is_empty A (o : option A) : bool :=
  match o with
  | None => true
...
Definition get_default A (o: option A) (a : A) : A :=
  match o with
  | None => a
  | Some x => x
  end.
...
Eval compute in tl (true :: false :: nil).
...
Fixpoint append A (l1 : list A) (l2 : list A) : list A :=
  match l1 with
  | nil => l2
  | x :: xs => x :: (append xs l2)
  end.
...
| Call : (non_positive -> False) -> non_positive.
...
*)
Fixpoint
  fold A B (f : B -> A -> A) (l : list B) (a : A) : A
:=
  match l with
  | nil => a
...
Fixpoint
  mem A {e : Eq A} (y : A) (l : list A) : bool
:=
  match l with
  | nil => false
...
Instance
  pair_Eq A (e1 : Eq A) B (e2 : Eq B) : Eq (A * B)
:=
  fun (x y : A * B) =>
...
:=
  ...
  | nil => None
  | (x,v) :: xs =>
	   if x == y then Some v else find y xs
  end.
